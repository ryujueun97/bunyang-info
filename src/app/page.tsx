...
import { 
  ...
  MapPin, Building, Filter, CheckCircle2, Clock, AlertCircle, RefreshCw, Navigation, ExternalLink
} from 'lucide-react';
...
const PROPERTIES: Property[] = [
  ...
  },
  {
    ...
    areaSize: '900,000m²',
    price: '분양 완료',
    description: '나노 융합 기술 생태계 조성 완료 단지.'
  },
  {
    id: '4',
    title: '인천 계양 지식산업센터',
    ...
    status: '분양예정',
    category: '지식산업센터',
    address: '인천광역시 계양구 서운동',
    image: 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&w=600&q=80',
    lat: 25,
    lng: 30,
    areaSize: '45,000m²',
    price: '상담 후 결정',
    description: '수도권 제1순환고속도로 접근성 우수, IT/지식기반산업 입지.'
  },
  {
    id: '5',
    title: '평택 고덕 첨단복합단지',
...
const CITIES_MAP: Record<string, string[]> = {
  ...
  '전라남도': ['전체', '장성군', '여수시', '순천시', '목포시'],
  '인천광역시': ['전체', '계양구', '연수구', '서구', '남동구']
};
...
export default function BunyangPage() {
  ...
  const getStatusBadge = (status: Property['status']) => {
    switch (status) {
      case '분양중':
        return <span className="bg-cyan-500 text-white font-bold text-xs px-2.5 py-1 rounded-full flex items-center gap-1"><CheckCircle2 className="w-3 h-3"/> 분양중</span>;
      case '분양예정':
        return <span className="bg-amber-500 text-white font-bold text-xs px-2.5 py-1 rounded-full flex items-center gap-1"><Clock className="w-3 h-3"/> 분양예정</span>;
      case '분양완료':
        return <span className="bg-slate-400 text-white font-bold text-xs px-2.5 py-1 rounded-full flex items-center gap-1"><AlertCircle className="w-3 h-3"/> 분양완료</span>;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col font-sans">
            ...
            <div className="flex-1 overflow-y-auto py-2">
              <div className="border-b border-slate-100">
                <button 
                  ...
                  className="w-full px-5 py-3.5 flex items-center justify-between font-bold text-red-600 hover:bg-red-50 transition"
                >
                  <span>회사소개</span>
                  {activeSubMenu === 'company' ? <ChevronDown className="w-4 h-4"/> : <ChevronRight className="w-4 h-4"/>}
                </button>
                {activeSubMenu === 'company' && (
                  <div className="bg-slate-50 py-1 px-8 text-sm space-y-2 text-slate-600 border-inner">
                    <a href="#ceo" className="block py-1.5 hover:text-red-600 transition">CEO 인사말</a>
                    <a href="#vision" className="block py-1.5 hover:text-red-600 transition">Motto & Vision</a>
                    <a href="#organization" className="block py-1.5 hover:text-red-600 transition">조직도 및 시스템</a>
                    <a href="#about" className="block py-1.5 hover:text-red-600 transition">새날그룹 소개</a>
                  </div>
                )}
              </div>
      ...
      {/* 4. DETAIL POPUP MODAL (산업단지 내용 팝업) */}
      {selectedProperty && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div 
            className="fixed inset-0 bg-black/60 backdrop-blur-sm"
      ...
      {/* 5. MAIN CONTENT MAP & LIST */}
      <main className="flex-1 flex flex-col lg:flex-row max-w-7xl w-full mx-auto p-4 gap-6">
        
        {/* LEFT / TOP MAP */}
        <section className="flex-1 bg-gradient-to-b from-slate-100 to-slate-200 border border-slate-300 rounded-2xl p-4 shadow-inner relative min-h-[480px] lg:min-h-[620px] flex flex-col">
          ...
          <div className="flex items-center justify-between mb-2 z-10 bg-white/90 backdrop-blur-md p-3 rounded-xl border border-slate-200 shadow-sm">
            <div>
              ...
              <p className="text-xs text-slate-500">붉은선: 도 단위 경계 | 핀 클릭 시 산업단지 내용 팝업</p>
            </div>
          ...
          <div className="relative flex-1 w-full h-full flex items-center justify-center overflow-hidden rounded-xl bg-slate-50/50">
            ...
            <svg 
              viewBox="0 0 500 700" 
              className="w-full h-full max-h-[580px] drop-shadow-md select-none"
            >
              <g stroke="#EF4444" strokeWidth="2.5" strokeLinejoin="round" fill="#F8FAFC" className="transition-all">
                <path 
                  ...
                  className={`cursor-pointer hover:fill-red-100 transition ${selectedProvince === '경기도' ? 'fill-red-200 stroke-red-700 stroke-[3.5]' : ''}`}
                  onClick={() => handleProvinceSelect('경기도')}
                />
                <path 
                  d="M210,110 L340,90 L380,180 L250,210 L250,140 Z" 
                  className={`cursor-pointer hover:fill-red-100 transition ${selectedProvince === '강원특별자치도' ? 'fill-red-200 stroke-red-700 stroke-[3.5]' : ''}`}
                  onClick={() => handleProvinceSelect('강원특별자치도')}
                />
                <path 
                  d="M230,200 L300,210 L280,300 L210,280 L210,230 Z" 
                  className={`cursor-pointer hover:fill-red-100 transition ${selectedProvince === '충청북도' ? 'fill-red-200 stroke-red-700 stroke-[3.5]' : ''}`}
                  onClick={() => handleProvinceSelect('충청북도')}
                />
                <path 
                  d="M140,180 L210,230 L210,280 L150,330 L100,260 Z" 
                  className={`cursor-pointer hover:fill-red-100 transition ${selectedProvince === '충청남도' ? 'fill-red-200 stroke-red-700 stroke-[3.5]' : ''}`}
                  onClick={() => handleProvinceSelect('충청남도')}
                />
                <path 
                  d="M300,210 L380,180 L420,340 L340,390 L280,300 Z" 
                  className={`cursor-pointer hover:fill-red-100 transition ${selectedProvince === '경상북도' ? 'fill-red-200 stroke-red-700 stroke-[3.5]' : ''}`}
                  onClick={() => handleProvinceSelect('경상북도')}
                />
                <path 
                  d="M280,300 L340,390 L320,460 L230,440 L250,370 Z" 
                  className={`cursor-pointer hover:fill-red-100 transition ${selectedProvince === '경상남도' ? 'fill-red-200 stroke-red-700 stroke-[3.5]' : ''}`}
                  onClick={() => handleProvinceSelect('경상남도')}
                />
                <path 
                  d="M150,330 L210,280 L250,370 L180,420 L130,370 Z" 
                  className={`cursor-pointer hover:fill-red-100 transition ${selectedProvince === '전라북도' ? 'fill-red-200 stroke-red-700 stroke-[3.5]' : ''}`}
                  onClick={() => handleProvinceSelect('전라북도')}
                />
                <path 
                  d="M130,370 L180,420 L230,440 L200,530 L100,500 Z" 
                  className={`cursor-pointer hover:fill-red-100 transition ${selectedProvince === '전라남도' ? 'fill-red-200 stroke-red-700 stroke-[3.5]' : ''}`}
                  onClick={() => handleProvinceSelect('전라남도')}
                />
                <path 
                  d="M110,580 L170,580 L160,620 L100,610 Z" 
                  className={`cursor-pointer hover:fill-red-100 transition ${selectedProvince === '제주특별자치도' ? 'fill-red-200 stroke-red-700 stroke-[3.5]' : ''}`}
            ...
            {filteredProperties.map((p) => (
              ...
              >
                <div className="flex flex-col items-center">
                  <div className="bg-white/90 backdrop-blur-md border border-slate-300 shadow-xl rounded-xl p-2 flex flex-col items-center gap-1 transition transform group-hover:scale-110 group-hover:border-red-500">
                    {getStatusBadge(p.status)}
                    <span className="font-extrabold text-xs text-slate-800 whitespace-nowrap px-1">
                      {p.title}
                    </span>
                  </div>
          ...
          <div className="mt-2 flex items-center justify-between bg-white/70 backdrop-blur-sm p-2 rounded-xl text-xs font-semibold text-slate-600 border border-slate-200">
            <div className="flex items-center gap-3">
            ...
            <span className="text-[11px] text-slate-400">* 핀 클릭 시 산업단지 상세 팝업</span>
          </div>
        </section>
              ...
              {['전체', '분양중'].map(st => (
                <button
                  ...
                  className={`text-xs px-2 py-1 rounded font-bold transition ${
                    selectedStatus === st ? 'bg-slate-900 text-white' : 'bg-slate-200 text-slate-600'
                  }`}
                >
                  {st}
                </button>
